	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				-- MODIFIED: AddOptions now expects each option to be a table: {Image = ..., TopText = ..., BottomText = ...}
				local function AddOptions(Options)
					for _, Option in pairs(Options) do
						-- Option can be a string or a table
						local imageId, topText, bottomText
						if type(Option) == "table" then
							imageId = Option.Image or ""
							topText = Option.TopText or tostring(Option[1] or "")
							bottomText = Option.BottomText or tostring(Option[2] or "")
						else
							imageId = ""
							topText = tostring(Option)
							bottomText = ""
						end

						local OptionBtn = AddThemeObject(SetProps(MakeElement("Button"), {
							Parent = DropdownContainer,
							Size = UDim2.new(1, 0, 0, 48), -- Larger height for option
							BackgroundTransparency = 1,
							ClipsDescendants = true
						}), "Divider")

						-- OptionFrame holds the image and text
						...
						local ImageLabel = SetProps(MakeElement("Image", imageId), {
							Parent = OptionFrame,
							Size = UDim2.new(0, 32, 0, 32),
						...
						local TopLabel = AddThemeObject(SetProps(MakeElement("Label", topText, 18), {
							Parent = TextHolder,
						...
						local BottomLabel = AddThemeObject(SetProps(MakeElement("Label", bottomText, 14), {
							Parent = TextHolder,
						...
						-- Add green stroke for click effect
						local GreenStroke = Instance.new("UIStroke")
						GreenStroke.Color = Color3.fromRGB(0, 255, 0)
						GreenStroke.Thickness = 2
						GreenStroke.Transparency = 1 -- Start invisible
						GreenStroke.Parent = OptionFrame
						OptionFrame.GreenStroke = GreenStroke -- FIX: Explicitly assign as property

						OptionBtn.OptionFrame = OptionFrame

						AddConnection(OptionBtn.MouseButton1Click, function()
							Dropdown:Set(type(Option) == "table" and (Option.Value or topText) or Option)
							if OrionLib.SaveCfg and DropdownConfig.Save then pcall(SaveCfg, game.GameId) end

							-- Animate green stroke: fade in, then fade out
							if OptionFrame and OptionFrame.GreenStroke then
								OptionFrame.GreenStroke.Transparency = 1
								local fadeIn = TweenService:Create(OptionFrame.GreenStroke, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {Transparency = 0})
								local fadeOut = TweenService:Create(OptionFrame.GreenStroke, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {Transparency = 1})
								fadeIn:Play()
						...
						Dropdown.Buttons[type(Option) == "table" and (Option.Value or topText) or Option] = OptionBtn
					end
				end	
				...
				function Dropdown:Set(Value)
					...
					for _, Option in pairs(Dropdown.Options) do
						local optValue = type(Option) == "table" and (Option.Value or Option.TopText or Option[1]) or Option
					...
					if not found then
						if #Dropdown.Options > 0 then
							local firstOpt = Dropdown.Options[1]
							Dropdown.Value = type(firstOpt) == "table" and (firstOpt.Value or firstOpt.TopText or firstOpt[1]) or firstOpt
						else
							Dropdown.Value = "..."
						end
						...
						for optKey, optButton in pairs(Dropdown.Buttons) do
							if optButton and optButton.Parent then
								...
								if optButton.OptionFrame and optButton.OptionFrame.TopLabel then TweenService:Create(optButton.OptionFrame.TopLabel,TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{TextTransparency = 0.4}):Play() end
								-- Hide green stroke if present
								if optButton.OptionFrame and optButton.OptionFrame.GreenStroke then
									optButton.OptionFrame.GreenStroke.Transparency = 1
								end
							end
						end	
					...
					if Dropdown.Buttons[Value] and Dropdown.Buttons[Value].Parent then
						...
						if Dropdown.Buttons[Value].OptionFrame and Dropdown.Buttons[Value].OptionFrame.TopLabel then TweenService:Create(Dropdown.Buttons[Value].OptionFrame.TopLabel,TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{TextTransparency = 0}):Play() end
						-- Hide green stroke except during animation (handled in MouseButton1Click)
					end
					return pcall(DropdownConfig.Callback, Dropdown.Value)
				end
				...
				AddConnection(Click.MouseButton1Click, function()
					...
					if Dropdown.Toggled then
						...
						if #Dropdown.Options > MaxElements then
							targetHeight = 38 + (MaxElements * 48) + paddingOffset -- 48 is new option height
						else 
